...
export class Mun extends DBTable {
	...
	async addScrip(addAmount, isTokens = false) {
		...
		await super.changeProperty(currencyType, this[currencyType])
		if (!isTokens) {
			this['lifetimeCapital'] += addAmount;
...
function sanitizeCurrency(scrip) {
	if (typeof scrip === 'string') {
		const output = scrip.replaceAll(',', '').trim();
