...
function displayShop(pageNum, shopType, category = "ALL") {
	...
	const allItems = shop.map((item) => ({
		label: item.name,
		value: item.name,
		description: `${item.buyPrice} ${item.currency} | ${item.type || "???"}`,
	}));
	...
	const shopItemsContainer = new ContainerBuilder().setAccentColor(embedColour(true));
