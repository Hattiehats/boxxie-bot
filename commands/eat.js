...
async function mainFunction(interaction) {
	...
	if (!thisItem.useText || thisItem.useText.trim() === '') {
		await interaction.editReply({
			components: simpleComponent("This item can't be eaten! ❌"),
			flags: MessageFlags.IsComponentsV2,
		});
		return;
	}
...
/*export default {
  ...
  async executePrefix(message, args) {
    if (!args) {
      await message.reply('Usage: `!eat <item>`');
      return;
    }
    ...
    if (!thisItem.useText || thisItem.useText.trim() === '') {
      await message.reply("This item can't be eaten! \u274C");
      return;
    }
    try {
      const outcome = await inventory.useItem(itemName);
	...
	? `\n-# (1x) ${itemName} was consumed.`
	: `\n-# \u267B\uFE0F This item is reusable; you did not lose it.`;
      const embed = basicEmbed(`Ate ${itemName}!`, `>>> ${outcome.text}${consumeText}`, thisItem.image || '');
